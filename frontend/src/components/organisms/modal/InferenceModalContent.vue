...
<template>
    <div class="flex flex-col justify-center items-center w-full gap-2 overflow-y-auto z-40">
        <div name="user=profile-container" class="flex content-center gap-3 max-md:flex-col items-center">
            <PaticipateProfile :image-url="userInfo?.profileUrl" custom-class="max-md:w-"></PaticipateProfile>
            <div name="inference-container" class="flex flex-col justify-between md:h-[200px]">
                <div class="md:w-[380px]">
                    <span class="text-A805Blue text-[16pt]"> {{ userInfo?.nickname }}</span> 님을
                    마니또로 예측하시겠습니까?
                </div>
                <div name="buttons-container" class="flex justify-between">
                    <ButtonAtom class="button-style-4 w-[100px] button-blue" :class="{ 'bg-A805Blue': inference === 'O' }"
                        @button-click="inferenceChanged('O')">O</ButtonAtom>
                    <ButtonAtom class="button-style-4 w-[100px] button-claret"
                        :class="{ 'bg-A805Violet': inference === 'X' }" @button-click="inferenceChanged('X')">X</ButtonAtom>
                    <ButtonAtom class="button-style-4 w-[100px] button-claret"
                        :class="{ 'bg-A805Violet': inference === '?' }" @button-click="inferenceChanged('?')">?</ButtonAtom>
                </div>
            </div>
        </div>
