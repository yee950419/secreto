    ...
    <div class="flex flex-col bg-A805RealWhite overflow-hidden border-b-2 border-A805DarkGrey">
        ...
        >
            ...
            <div class="flex justify-between gap-3">
                <p class="text-A805Blue" @click="usersApproveHandler">선택 승인</p>
                <p @click="usersDeniedHandler">선택 거절</p>
            </div>
        </div>
        <div v-for="user in userCheckList" :key="user.id" class="px-4 py-2">
            <div class="flex items-center justify-between">
                ...
                <div class="flex gap-5 text-[32px]">
                    <CheckOutlined class="text-A805Green" @click="$emit('usersApproved', [user])" />
                    <MinusCircleOutlined
                        class="text-A805Red"
